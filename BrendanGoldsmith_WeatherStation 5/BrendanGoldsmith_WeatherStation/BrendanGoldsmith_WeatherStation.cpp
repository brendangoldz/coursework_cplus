...
void insert(MeasureWeather * test) {
	for (int i = m; i > 0; i--) {
		test[i] = test[i - 1];
	}
	test[0].getMeasure();
...
void print(MeasureWeather * n4, int a) {

	if (a == 0) {
		cout << endl << " The " << name << " Weather Station" << endl;
		cout << "Current Temperature (*F): " << n4[0].getMeasure().temp << endl;
		cout << "Wind Speed & Direction: " << n4[0].getMeasure().wind.speed << n4[0].getMeasure().wind.dir <<endl;
	}
	else {
		if (m < x) {
			for (int j = 0; j < m ; j++) {
				cout << endl << " The " << name << " Weather Station" << endl;
				cout << "Current Temperature (*F): " << n4[j].getMeasure().temp << endl;
				cout << "Wind Speed & Direction: " << n4[j].getMeasure().wind.speed << " " << n4[j].getMeasure().wind.dir << endl;
			}
		}
		else {
			for (int j = 0; j < x; j++) {
...
{
	int size = splash();
	MeasureWeather * pmeasure = new MeasureWeather[size];
